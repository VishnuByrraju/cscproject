...
const fetchUserProfile = async () => {
  ...
  const handleUpdateProfile = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${BASE_URL}/v1/edit-profile/${userMail}/${userRole}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${accesstoken}`,
          ...
          <div className="flex flex-col lg:flex-row lg:space-x-8">
            ...
            <div className="w-full lg:w-1/2">
              ...
              {updatePasswordSuccess && (
                <PopupComponent content={updatePasswordSuccess} contentType={"Update Password Status"} redirectPath={""}/>
              )}
              {updatePasswordError && (
                <PopupComponent content={updatePasswordError} contentType={"Update Password Status"} redirectPath={""}/>
              )}
            </div>
          </div>
        </div>
